# MLOps Pipeline v2 — Diagnóstico de Enfermedades
...
![Pipeline MLOps](./entrega3/mlops-pipeline-v2.png)
...
- **Tecnología frontend:** React (SPA) + FastAPI client para el estado actual del diagrama, utilizando Fetch API o Axios para la comunicación con el backend. El prototipo base puede iniciarse con HTML/CSS/JavaScript estático + Fetch API, migrando a React cuando se requieran múltiples pantallas, autenticación y estado clínico complejo.
...
| Cómputo alternativo | AWS EC2 / ECS (opción más simple antes de adoptar EKS) |
...
> **Suposición:** dado que las enfermedades huérfanas implican pocos pacientes por definición, **la opción A (local) es viable en etapas tempranas**. La opción B se recomienda una vez el modelo está validado clínicamente y se necesita escalar a múltiples hospitales o centros médicos. En la transición hacia cloud, EC2/ECS representa un escalón intermedio antes de adoptar la orquestación completa con EKS.
...
| **Python + Faker + hashlib (SHA-256)** | Anonimización y generación de datos sintéticos de prueba | Los identificadores del paciente (nombre, ID) se reemplazan por hashes SHA-256 antes de almacenar. Faker se utiliza para generar datos sintéticos realistas en entornos de desarrollo y testing, sin exponer datos reales de pacientes. El hash SHA-256 no es reversible y cumple con requisitos de privacidad clínica. |
...
### Gate de salida — ¿Cumple el recall mínimo?

Al concluir el ciclo de experimentos se evalúa explícitamente si el modelo seleccionado cumple los umbrales de recall definidos:

- **Sí cumple:** el modelo seleccionado pasa a la **Etapa 4** (tests unitarios) y continúa el pipeline hacia producción.
- **No cumple:** se regresa a la Etapa 2 para ajustar la estrategia de aumentación, revisar el encoding o explorar nuevas arquitecturas. El equipo revisa manualmente las métricas en MLflow antes de iterar.

> **Nota:** este gate es obligatorio y bloquea el avance al pipeline de producción. Ningún modelo que no supere los umbrales de recall pasa a las etapas de CI/CD.
...
Si recall < umbral → alerta → Trigger de reentrenamiento
    ↓
Si se detecta posible concept drift → revisión manual en Etapa 3
```
...
- Si el monitoreo señala **posible concept drift**, el equipo puede activar manualmente el Trigger y regresar directamente a la **Etapa 3** para re-evaluar la estrategia de modelado antes de ejecutar el pipeline de producción.
...
| Gate de recall mínimo | Etapa 3 → Etapa 4 (o regreso a E2) | Recall cumple o no los umbrales antes de pasar a CI/CD | Evaluación explícita post-experimentos |
| Reentrenamiento | Monitoreo → Trigger → Etapa 5 | Degradación del recall en producción | Airflow `TriggerDagRunOperator` |
| Revisión por concept drift | Monitoreo → Etapa 3 | Degradación abrupta o patrón atípico de recall en producción | Airflow + alerta manual al equipo |
...
| Anonimización y datos de prueba | Python Faker + hashlib (SHA-256) | Datos |
...
| Cómputo cloud alternativo | AWS EC2 / ECS | Producción |
...
|---|---|---|---|
| — | Arquitectura global | **Nuevo:** arquitectura bifurcada local/cloud con Docker Compose (local) y AWS EKS (cloud). **Nuevo:** EC2/ECS documentado como escalón intermedio en la transición hacia EKS | Se requieren definir dos modelos de despliegue; EC2/ECS aparece en el diagrama de arquitectura como opción cloud complementaria |
| 1 | Centralización | Tecnología especificada: **Airflow + PostgreSQL + Python Faker + SHA-256** para anonimización y generación de datos de prueba | v1 no especificaba el orquestador ni el mecanismo de anonimización; Faker aparece en el diagrama como herramienta complementaria a hashlib |
| 1.1 | Puntos de entrada — Frontend | **Actualizado:** React + FastAPI client como tecnología del estado actual del diagrama, no solo como recomendación futura | El diagrama muestra React + FastAPI client en el bloque de entrada de datos como tecnología presente |
...
| 3 | Experimentos DS | **Especificado:** MLflow para tracking, Optuna para hiperparámetros. **Nuevo:** umbrales de recall cuantificados (0.75 / 0.85). **Nuevo:** justificación de no usar AutoML. **Nuevo:** gate explícito "¿Cumple el recall mínimo?" como paso obligatorio antes de pasar a CI/CD | v1 mencionaba Git y modelos pero sin tracking, umbrales numéricos ni gate de salida formalizado |
...
| 10 | Monitoreo | **Especificado:** Prometheus + Grafana + `prometheus-fastapi-instrumentator`. **Nuevo:** ventana deslizante de 30 días para el recall. **Nuevo:** período de burn-in de 2 semanas. **Nuevo:** detección y manejo de posible concept drift como señal diferenciada del reentrenamiento rutinario | v1 describía el ciclo de monitoreo correctamente pero sin tecnologías, parámetros temporales ni distinción entre degradación gradual y concept drift |
| — | Trigger | **Nuevo:** condición de 3 días consecutivos bajo umbral para evitar falsos positivos. **Nuevo:** reentrenamiento periódico preventivo cada 90 días. **Especificado:** `TriggerDagRunOperator` de Airflow. **Nuevo:** ruta manual hacia Etapa 3 cuando se detecta concept drift | v1 mencionaba el Trigger pero sin condición de activación precisa, tecnología, ni distinción de rutas según tipo de degradación |
